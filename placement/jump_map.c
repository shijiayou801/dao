...
retry:
	do {
		...
		if (curr_dom->do_children == NULL) {
			...
			struct pool_target *t = *target;
			fprintf(stderr, "get_target picked " DF_TARGET "\n",
					DP_TARGET(t));
			...
			if (range_set) {
				...
				fprintf(stderr, "dom %p %d used up\n",
			...
			if (range_set) {
				if (top == -1) {
					fprintf(stderr, "domain range has been used by the current group, start=%lu end=%lu\n", start_dom, end_dom); 
		 ...
		 uint32_t failed_in_layout, bool *is_extending)
{
	...
	while (current != remap_list) {
		...
		fprintf(stderr,  "Attempting to remap failed shard: "
		...
		if (spare_avail) {
			...
			fprintf(stderr, "Trying new target: "DF_TARGET"\n",
				DP_TARGET(spare_tgt));
			spares_left--;
		}
		  ...
		  bool *is_extending)
{
	...
	fprintf(stderr, "get object layout called\n");
	...
	for (i = 0, k = 0; i < jmop->jmop_grp_nr; i++) {
		...
		for (j = 0; j < jmop->jmop_grp_size; j++, k++) {
			...
			if (need_remap_target(target, allow_status, allow_version)) {
				...
				fprintf(stderr, "Target unavailable " DF_TARGET
					". Adding to remap_list: fail cnt %d\n",
					DP_TARGET(target), fail_tgt_cnt);
		   ...
		   struct pl_obj_layout **layout_pp)
{
	...
	if (unlikely(is_extending || is_adding_new) && !(mode & DAOS_OO_RO)) {
		...
		fprintf(stderr, DF_OID"/%d is being added: %s or extended: %s\n",
			   ...
			   unsigned int array_size)
{
	...
	fprintf(stderr, "find addition first get layout\n");
	allow_status = PO_COMP_ST_UPIN;
	...
	fprintf(stderr, "find addition second get layout\n");
	allow_status |= PO_COMP_ST_NEW;
	rc = obj_layout_alloc_and_get(jmap, &jop, md, allow_status, reint_ver,
